...
public class SourceConfig {
    ...
    /**
     * These are dev inf extensions that the source can add to the global dev
     * info object
     */
    ...
    public void setLastAnchor(long anchor) {
        if (Log.isLoggable(Log.DEBUG)) {
            Log.debug(TAG_LOG, "[ANCHOR]" + name + " setting last anchor to " + anchor);
        }
    ...
    public void setNextAnchor(long anchor) {
        if (Log.isLoggable(Log.DEBUG)) {
            Log.debug(TAG_LOG, "[ANCHOR] "+ name +" setting next anchor to " + anchor);
        }

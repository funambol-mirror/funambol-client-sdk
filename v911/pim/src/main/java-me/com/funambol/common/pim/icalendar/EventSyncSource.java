...
{
    private static final String TAG_LOG = "EventSyncSource";
    ...
    protected byte[] formatItem(PIMItem item) throws PIMException {
        ...
        if (Log.isLoggable(Log.TRACE)) {
            Log.trace(TAG_LOG, "Formatted item: ");
            Log.trace(TAG_LOG, vCard);
        }
        try {
            return vCard.getBytes("UTF-8");
        } catch (Exception e) {
            Log.error(TAG_LOG, "Cannot get UTF-8 bytes from string", e);
    ...
    protected void fillItem(PIMItem pitem, String content) throws PIMException {
        ...
        try {
            ...
            Log.error(TAG_LOG, "Exception while parsing the item ", pe);

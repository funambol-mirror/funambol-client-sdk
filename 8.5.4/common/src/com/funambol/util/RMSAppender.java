...
public class RMSAppender implements Appender {
    ...
    public static final int LOGSIZETOBEFREE = DEFAULTLOGFILESIZE/2;
    
    // if loglevel is debug the RMS Recordstore is doubled
    public static final int EXTENDED_SIZE = 2 * DEFAULTLOGFILESIZE;
    ...
    public RMSAppender(String logfilename) {
        ...
        this.defaultLogStoreSizeToBeFree = LOGSIZETOBEFREE;
        this.firstRecord = 1;
    }
    //----------------------------------------------------------- Public methods
    /**
    ...
    public void setLogLevel(int newlevel) {
        if (newlevel == Log.DEBUG) {
            setDefaultLogFileSize(EXTENDED_SIZE);
        } else {
            setDefaultLogFileSize(DEFAULTLOGFILESIZE);
        }
    }
    ...
    public LogContent getLogContent() throws IOException {
        ...
        return new LogContent(LogContent.FILE_CONTENT, log.toString());
    }
}

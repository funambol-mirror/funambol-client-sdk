...
public abstract class Customization {
    ...
    protected final String   ABOUT_COPYRIGHT_DEFAULT = "Copyright " + (char)169 + " 2009 - 2010";
    ...
    protected final boolean  ENABLE_UPDATER_MANAGER  = false; // Used for Funambol update
    ...
    // Specifies if the login screen must be presented when the application
    // starts and/or credentials are invalid
    private final boolean  LOGIN_SCREEN_REQUIRED        = true;

    // Specifies if the account screen shall check credentials on the server
    // when the user changes them (this is done via config synchronization)
    private final boolean  CHECK_CRED_IN_ACCOUNT_SCREEN = true;
    ...
    // Sync items type
    private final String DEFAULT_CALENDAR_TYPE = "text/vcal";
    ...
    private final String   DEFAULT_PICTURES_DIR        = "file:///store/home/user/pictures/";
    ...
    private final boolean  BLOCK_INVITES_FIELD_VISIBLE = true;
    ...
    private final boolean  HIDE_HELP_SCREEN            = false;
    ...
    protected final boolean CONTACTS_IMPORT_ENABLED = true;
    ...
    public boolean loginScreenRequired() {
        return LOGIN_SCREEN_REQUIRED;
    }

    public boolean checkCredentialsInAccountScreen() {
        return CHECK_CRED_IN_ACCOUNT_SCREEN;
    }
    ...
    public boolean isSourceEnabledByDefault(int id) {
        boolean enabled = ((Integer)sourcesSyncMode.get(new Integer(id))).intValue()

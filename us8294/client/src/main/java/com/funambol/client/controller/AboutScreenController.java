...
public class AboutScreenController {
    ...
    public void addNecessaryFields() {
        ...
        // Add portal info
        if (customization.showPortalInfo()) {
            String portalURL = customization.getPortalURL();
            String portalInfo = localization.getLanguage("portal_info");
            portalInfo = StringUtil.replaceAll(portalInfo, "__PORTAL_URL__", portalURL);
            if (portalInfo != null && portalInfo.length() > 0) {
                aboutScreen.addPortalInfo(portalInfo);
            }
        }

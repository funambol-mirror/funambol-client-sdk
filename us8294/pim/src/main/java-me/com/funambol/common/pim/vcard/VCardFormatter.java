...
public class VCardFormatter {
    ...
    private static final String TAG_LOG = "VCardFormatter";
    ...
    public void format(Contact contact, OutputStream os, boolean allFields) throws PIMException {
        if (Log.isLoggable(Log.INFO)) {
            Log.info(TAG_LOG, "Formatting VCard");
        }
        ...
        if (Log.isLoggable(Log.INFO)) {
            Log.info(TAG_LOG, "Formatting VCard done");
        }
    }
    ...
    protected void formatFields(Contact contact, OutputStream os, boolean allFields) throws PIMException {
        ...
        if (Log.isLoggable(Log.DEBUG)) {
            Log.debug(TAG_LOG, "Formatting Version");
        }
        formatVersion(contact, os);

        if (Log.isLoggable(Log.DEBUG)) {
            Log.debug(TAG_LOG, "Formatting Name");
        }
        ...
        if (Log.isLoggable(Log.DEBUG)) {
            Log.debug(TAG_LOG, "Formatting Telephone");
        }
        ...
        if (Log.isLoggable(Log.DEBUG)) {
            Log.debug(TAG_LOG, "Formatting Address");
        }
        ...
        if (Log.isLoggable(Log.DEBUG)) {
            Log.debug(TAG_LOG, "Formatting Birthday");
        }
        ...
        if (Log.isLoggable(Log.DEBUG)) {
            Log.debug(TAG_LOG, "Formatting Email");
        }
        ...
        if (Log.isLoggable(Log.DEBUG)) {
            Log.debug(TAG_LOG, "Formatting Title");
        }
        ...
        if (Log.isLoggable(Log.DEBUG)) {
            Log.debug(TAG_LOG, "Formatting Url");
        }
        ...
        if (Log.isLoggable(Log.DEBUG)) {
            Log.debug(TAG_LOG, "Formatting Org");
        }
        ...
        if (Log.isLoggable(Log.DEBUG)) {
            Log.debug(TAG_LOG, "Formatting Note");
        }
        ...
        if (Log.isLoggable(Log.DEBUG)) {
            Log.debug(TAG_LOG, "Formatting Photo");
        }
        ...
        if (Log.isLoggable(Log.DEBUG)) {
            Log.debug(TAG_LOG, "Formatting Categories");
        }
    ...
    throws PIMException
    {
        ...
        if (contact.countValues(Contact.PHOTO) > 0) {
            try {
                ...
                Log.error(TAG_LOG, "Error formatting the photo", e);
            }
        }
            ...
            if (value != null) {
                ...
                switch (type) {
                    ...
                    case Contact.ATTR_OTHER:
                        ...
                        break;
                    default:
                        Log.error(TAG_LOG, "Unsupported telephone type");
                }
            }
        }
    ...
    throws PIMException
    {
        ...
        for(int i=0;i<contact.countValues(Contact.EMAIL);++i) {
            if (Log.isLoggable(Log.DEBUG)) {
                Log.debug(TAG_LOG, "Fetching email number: " + i);
            }

    ...
    private void computeVisibleItems() {
        ...
        if(sourceSettingsUIItems == null) {
            ...
            if(customization.showSyncModeInSettingsScreen()) {
                miscSettingsUIItems.add(ssScreen.addSyncModeSetting());
                miscSettingsUIItems.add(ssScreen.addSyncIntervalSetting());
            }

            // Add C2S push setting if required
            if(customization.showC2SPushInSettingsScreen()) {
                miscSettingsUIItems.add(ssScreen.addC2SPushSetting());
            }
        ...
        if(sourceSettingsUIItems != null) {
            ...
            while(items.hasMoreElements()) {
                ...
                if(item.isWorking()) {
                    ...
                    if(item.hasSetting(AppSyncSource.SYNC_MODE_SETTING)) {

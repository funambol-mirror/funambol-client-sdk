...
public class CTPService extends Thread {
    ...
    private boolean hasBeenStoppedForReal;
    ...
    public void restartService() {
        if (this.config != null) {
            ...
            Log.error("[CTPService] Cannot restart service: current config is null");
        }
    }
    ...
    public void stopService(boolean wait) {
        ...
        Log.debug("stopservice ctp service stopping with wait = " + wait);
            if (instanceRunning) {
                hasBeenStoppedForReal = false;
                stopService();
                if (wait) {
                    Log.debug("waiting for service to stop");
                    /*try {
                    lock.wait();
                    ...
                    int i = 0;
                    while (hasBeenStoppedForReal == false) {
                        try {
                            Thread.sleep(1000);
                            Log.debug("sleeeep");
                        /*  i++;
                        if (i > 50) throw new InterruptedException();*/
                        } catch (InterruptedException ex) {
                            // it's ok, it's exactly what we want, i suppose
                            ex.printStackTrace();
                        }
                ...
                Log.debug("stopservice:  CTP is not running, nothing to do!");
            }
        }
    ...
    public void stopService() {
        ...
        try {
            ...
            Log.debug("instancerunning set to false");
        }
            ...
            try {
                ...
                if (state == CONNECTED) {
                    ...
                    if ((authStatus == ST_UNAUTHORIZED) ||
                            ...
                            (authStatus == ST_NOT_AUTHENTICATED)) {
                        // No point in trying again
                        Log.debug("authstatus is  " + authStatus + " no point in trying again");
                        done = true;
                        disconnect();
     ...
     */
    private void connect(int retry) throws IOException {
    ...
    private void disconnect() {
        if (state != DISCONNECTED) {
            ...
            if (hasBeenStoppedForReal == false) {
                hasBeenStoppedForReal = true;
            }
    ...
    public void forceDisconnect() {
        ...
        // we need this to be sure that if the thread is sleeping waiting for 
        // the flag to be set to true it can termitate
        hasBeenStoppedForReal = true;
    }
     ...
     */
    private synchronized int authenticate() throws IOException {
        ...
        switch (authStatus) {
            ...
            case ST_NOT_AUTHENTICATED:
                ...
                if (authStatus == ST_OK) {
                    ...
                    Log.debug("[CTPService] received authstatus " + authStatus);

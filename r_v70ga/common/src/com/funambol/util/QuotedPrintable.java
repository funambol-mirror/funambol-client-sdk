...
public class QuotedPrintable {
     ...
     */
     ...
     public static String encode(String stringToEncode, String enc){
       String encoded ="";
       
        for(int i = 0; i<stringToEncode.length();i++){
            char ch = stringToEncode.charAt(i);
            Character character  = new Character(ch);
            int dec = character.hashCode();
            
            if(dec > 125){
                if(encoded.indexOf("=?"+enc+"?Q?")== -1){
                    encoded += "=?"+enc+"?Q?";
                }
                String toAppend= Integer.toHexString(dec); 
                //FIX FOR € char
                if (StringUtil.equalsIgnoreCase(toAppend, "20ac")){
                    toAppend = "A4";
                }
                    
                encoded += '='+toAppend.toString().toUpperCase();
            }else{

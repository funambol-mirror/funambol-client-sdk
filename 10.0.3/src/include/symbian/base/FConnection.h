...
#include <apengineconsts.h>     // TApBearerType
 ...
 */
class FConnection : public CActive
{
...
private:
    ...
    /**
     * Used to block the execution when setting up the internet connection
     * until the connection is up (see startConnection method).
     * The connection is estabilished asynchronously via this active
     * object, so we simulate a synchronous action waiting on the OS response.
     */
    CActiveSchedulerWait*   iASWait;

    /**
     * The ID of the main thread.
     * @note  It's mandatory to originate and complete an asynchronous
     *        request inside the same thread, so here is the ID of the sole
     *        thread used to originate and complete requests.
     */
    TInt                    iMainThreadID;
     ...
     * It's called by the active scheduler when a request completion event occurs.
     * The active object is used to start a connection asynchronously, to avoid
     * freezing the UI during the connection startup process.
     */
    void RunL(void);
...
public:
    ...
    /**
     * Returns the type of connection (one of TApBearerType) for the desired APN name.
     * If no APN name is specified, returns the connection type for the current connection.
     * It can be used to know if a certain connection is CSD, GPRS, HSCSD, WLAN,...
     */
    const TApBearerType getConnectionType(const StringBuffer aIAPName = "");

    /**
     * Returns true if the passed APN name is WLAN (wifi).
     * If no APN name is specified, it checks the current connection.
     */
    const bool isWLANConnection(const StringBuffer aIAPName = "");


    /**
     * Closes and restarts the RSocketServ session.
     * It is used in case the session is stuck and always returns an
     * error (KErrNotReady = -18). Calling this method will cause the current
    ...
    void setIAPName(const StringBuffer& aIAPName) {
        iIAPName = aIAPName;
        iIAP = GetIAPIDFromName(aIAPName);
    }

    /**
     * Returns the current IAP name (the APN name currently used).
     * It doesn't say if the connection is currently used or not (for this
     * information use isConnected() method) but it says which is the
     * active IAP name.
     */
    ...
    /**
     * Returns an array of (new allocated) descriptors, with all
     * the current WLAN IAP names.
     * The caller has the ownership of the array, so he should take
     * care of deleting it (calling Close()).
     */
    RArray<HBufC*> GetWifiAPNames();

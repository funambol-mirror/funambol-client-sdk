...
FileOutputStream::FileOutputStream(const char* path, bool appendMode) : OutputStream() {
    ...
    if (appendMode) {
        ...
        if (f) {
            ...
            LOG.debug("[%s] file %s opened: append to byte %d", __FUNCTION__, path, bytesWritten);
...
int FileOutputStream::setPosition(size_t offset) {
    if (f) {
...
size_t FileOutputStream::getPosition() {
    if (f) {
        return ftell(f);
    }
    return 0;

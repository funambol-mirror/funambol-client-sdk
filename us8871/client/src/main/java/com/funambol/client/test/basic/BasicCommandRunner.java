...
public abstract class BasicCommandRunner extends CommandRunner implements BasicUserCommands {
    ...
    public boolean runCommand(String command, Vector pars) throws Throwable {
        ...
        } else if (SET_VARIABLE.equals(command)) {
            setVariable(command, pars);
        } else {
            return false;
        }
        return true;
    ...
    private void setVariable(String command, Vector args) throws Throwable {
        String variable = getParameter(args, 0);
        String value = getParameter(args, 1);
        getBasicRobot().setVariable(variable, value);
    }

...
public class MetadataBusProxy implements TableObserver {
    ...
    public MetadataBusProxy(AppSyncSource source) {
        this.source = source;
    }

    public void tupleInserted(Tuple tuple) {
        if (Log.isLoggable(Log.TRACE)) {
            Log.trace(TAG_LOG, "Tuple inserted");
        }
        BusMessage msg = new MetadataBusMessage(source,
                MetadataBusMessage.ACTION_INSERTED, tuple);
        bus.sendMessage(msg);
    }

    public void tupleUpdated(Tuple tuple) {
        if (Log.isLoggable(Log.TRACE)) {
            Log.trace(TAG_LOG, "Tuple updated");
        }
        BusMessage msg = new MetadataBusMessage(source,
                MetadataBusMessage.ACTION_UPDATED, tuple);
        bus.sendMessage(msg);
    }

    public void tupleDeleted(Object key) {
        if (Log.isLoggable(Log.TRACE)) {
            Log.trace(TAG_LOG, "Tuple deleted");
        }
        BusMessage msg = new MetadataBusMessage(source,
                MetadataBusMessage.ACTION_DELETED, key);
        bus.sendMessage(msg);
    }

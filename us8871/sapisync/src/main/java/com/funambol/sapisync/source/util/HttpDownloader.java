...
public class HttpDownloader  {
     ...
     */
    public long download(String url, OutputStream os, String testName) throws DownloadException, IOException {
        boolean retry;
        int i = 0;
        ...
        do {
            ...
            try {
                if (downloadedSize == 0) {
                    ...
                    downloadedSize = download(url, os, -1, testName);
                } else {
                    // Try to resume previous download
                    if (Log.isLoggable(Log.INFO)) {
                        Log.info(TAG_LOG, "Resuming download at " + downloadedSize);
                    }
                    downloadedSize = download(url, os, downloadedSize, testName);
                }
     ...
     * @throws IOException
     */
    public long resume(String url, OutputStream os, long startOffset, String testName)
    throws DownloadException, IOException, ResumeException {
        return download(url, os, startOffset, testName);
    }
     ...
     */
    protected long download(String url, OutputStream os, long startOffset, String testItemName)
    throws DownloadException, IOException, ResumeException {
        ...
        try {
            ...
            if (Log.isLoggable(Log.DEBUG)) {
                Log.debug(TAG_LOG, "Response is: " + respCode);
            }

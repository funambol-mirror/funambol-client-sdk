...
public class SyncManager {
    ...
    public synchronized void sync(SyncSource src, int syncMode)
        ...
        sourceLOHandler = new SyncSourceLOHandler(src, maxMsgSize);
        ...
        try {
            ...
            Log.debug("[SyncManager] Response: " + response);
            ...
            Log.info("[SyncManager] Alert code: " + alertCode);
            ...
            try {
                ...
                Log.error("[SyncManager] Error parsing RespURI from server " + xe.toString());
            ...
            */
            ...
            do {
                ...
                mappingManager.saveMappings(source.getName(), mappings);
            ...
            if (!isMappingTestDisabled) {
                ...
                throw new SyncException(WriteRequestException.SERVER_CONNECTION_REQUEST_ERROR, "Connection blocked for test purposes");
            }
            ...
            //releaseResources();
            // TODO, we should grab the proper error and not such a generic one
            syncStatus = SyncSource.STATUS_CONNECTION_ERROR;
    ...
    private SyncItem getItem(char state, String type, ChunkedString xmlItem,
                             String[] formatList) throws SyncException {
        String key = null;
        String data = null;
        String parent = null;
        byte[] content = null;
    ...
    private SyncMLStatus processSyncItem(SyncMLCommand cmd, ChunkedString xmlItem, String [] formatList)
        ...
        if (cmdTag.equals(SyncML.TAG_ADD)) {
            ...
            if (item.getContent() != null) {
                status = sourceLOHandler.addItem(item);
            ...
            // for update
            if (item.getContent() != null) {
                status = sourceLOHandler.updateItem(item);
                if (SyncMLStatus.isSuccess(status)) {
    ...
    private int processCommand(ChunkedString msgRef, String cmdName, ChunkedString command)
        ...
        try {
            ...
            for (i = 0; i < len; i++) {
                ...
                // Allow the GC to pick up this memory
                itemTags.setElementAt(null, i);
            }
                ...
                if (XmlUtil.getTag(bodyTag, SyncML.TAG_SYNC) != - 1) {
                    ...
                    for (int c = 0; c < cmdNames.length; c++) {
                        ...
                        for (int i = 0,  l = cmdTags.size(); i < l; i++) {
                            ...
                            // Allow GC to pick the element
                            cmdTags.setElementAt(null, i);
    ...
    private String prepareModificationMessage() throws SyncException {
        ...
        if (msgID == 2) {
            modMsg.append("<Status>\n").append("<CmdID>").append(getNextCmdID())
                  .append("</CmdID>\n").append("<MsgRef>1</MsgRef><CmdRef>1</CmdRef><Cmd>Alert</Cmd>\n")
                  .append("<TargetRef>").append(source.getSourceUri())
                  .append("</TargetRef>\n").append("<SourceRef>")
                  .append(source.getSourceUri()).append("</SourceRef>\n")
                  .append("<Data>200</Data>\n").append("<Item>\n").append("<Data>\n")
                  .append("<Anchor xmlns=\"syncml:metinf\"><Next>")
                  .append(source.getNextAnchor()).append("</Next></Anchor>\n")
                  .append("</Data>\n").append("</Item>\n").append("</Status>\n");
        }
    ...
    private String createDevInf(DeviceConfig devInf) {
        ...
        sb.append("<DataStore>\n")//
          ...
          .append("<Rx-Pref>\n").append("<CTType>").append(source.getType())
          .append("</CTType>\n").append("<VerCT></VerCT>\n")
          .append("</Rx-Pref>\n") //required for each specified datastore
          .append("<Tx-Pref>\n").append("<CTType>").append(source.getType())
          .append("</CTType>\n").append("<VerCT></VerCT>\n").append("</Tx-Pref>\n") //SyncCap
    ...
    private String getNextCmdTag(int size) throws SyncException {
        ...
        switch (alertCode) {
            ...
            case SyncML.ALERT_CODE_SLOW:
            case SyncML.ALERT_CODE_REFRESH_FROM_CLIENT:
                boolean done = sourceLOHandler.getNextCommand(size, listener, cmdTag, cmdID);
                if (done) {
                    nextState(STATE_MODIFICATION_COMPLETED);
                }
    ...
    private String prepareSyncTag(int size) throws SyncException {
        ...
        syncTag.append("<Sync>\n").append("<CmdID>").append(getNextCmdID())
               .append("</CmdID>\n").append("<Target><LocURI>")
               .append(source.getSourceUri()).append("</LocURI></Target>\n")
               .append("<Source><LocURI>").append(source.getName())
    ...
    private int getSourceAlertCode(String sourceURI) {
        ...
        try {
            ...
            t.printStackTrace();
            Log.error("ERROR: unrecognized server alert code ("
    ...
    private String sendLastSyncMappingMessage(String response) throws SyncException {

        //TODO: enhance the listener interface to trace this changes
        String mappingMessage = prepareMappingMessage(false);
        Log.info("Sending mapping commands lost in the last sync");
        Log.debug(mappingMessage);

        response = postRequest(mappingMessage);
        ...
        Log.debug(response);
        //TODO: enhance the listener interface to trace this changes
        //TODO: must process the response
        return response;
    }

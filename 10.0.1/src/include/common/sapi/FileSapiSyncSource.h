...
public:
     ...
     */
    FileSapiSyncSource(SyncSourceConfig& sc, SyncSourceReport& report, size_t incomingFilterDate, size_t outgoingFilterDate, const char* storageLocation = NULL);
    ...
    * @return   > 0 means an error. <= 0 means all successfully
     *           -3: folder path not found in config
     *           -2: default implementation (basically not implemented)
     *           -1: item locally not found. It is considered as success by the way
     *            0: item removed successfully
     *            1: item not removed successfully 
     */

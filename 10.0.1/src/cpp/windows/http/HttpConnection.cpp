...
int HttpConnection::request(InputStream& data, OutputStream& response, bool log_request)
{
    ...
    DWORD resTimeoutMsec = responseTimeout* 1000;
    ...
    if (!sendDataAtOnce) {
        int bufferSize = min(requestChunkSize, contentLen);
...
int HttpConnection::readResponseHeaders()
{
    ...
    //
            ...
            DWORD dwError = HTTP_UNAUTHORIZED;
            ...
            if (dwError == ERROR_INTERNET_FORCE_RETRY) {
                return dwError;    // TODO: catch this error on upper level and retry!
            }
            else {

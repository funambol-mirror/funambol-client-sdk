...
public class SyncSourceHandlerTest extends TestCase {
    ...
    public void testChunkdecoding() throws Throwable {
        ...
        Chunk firstChunk = handler.getItem("text/plain", new ChunkedString(chunk0), null, null);
        Chunk secondChunk = handler.getItem("text/plain", new ChunkedString(chunk1), null, null);
    ...
    public void testWrongSourceParent() throws Throwable {
        ...
        // The parent key is not mapped, the sync command must fail
        try {
            simpleSyncItemTest(xmlCommand, "101", "xyz01", hierarchy);
            assertTrue(false);
        } catch(SyncException e) {
            // The SyncItem parent is not mapped -> OK
        }
    }

...
public class VAlarm {
     ...
     */
    public void setTriggerAbsoluteTime(String time) throws ParseException {
    ...
    public void setTriggerRelativeTime(String time) throws ParseException {
        ...
        Log.error("[CalendarParserListener.updateTrigger] Cannot convert ISO 8601 duration");
     ...
     * @throws com.funambol.common.pim.icalendar.ParseException
     */
    public void setCalEndAbsoluteTime(long time) throws ParseException {
        relatedEndTime = time;
    }
    ...
    public long getAlarmInterval() {
        if(alarmAbsoluteTime == CalendarUtils.UNDEFINED_TIME) {
            Log.error("[VAlarm.getAlarmInterval] The alarm absolute time hasn't been set yet");
        ...
        if(date == CalendarUtils.UNDEFINED_TIME) {
            Log.error("[VAlarm.getAlarmInterval] The alarm related absolute time hasn't been set yet");
            return CalendarUtils.UNDEFINED_TIME;
        }
    ...
    public boolean setAlarmInterval(long interval) {
        if(relatedStartTime == CalendarUtils.UNDEFINED_TIME) {
            Log.error("[VAlarm.setAlarmInterval] The alarm start time hasn't been set yet");
            return false;
        }
        if(interval>0) {
            ...
            Log.error("[VAlarm.setAlarmInterval] Invalid interval: " + interval);
            return false;
        }
    }
}

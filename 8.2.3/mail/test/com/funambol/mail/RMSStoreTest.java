...
public class RMSStoreTest extends TestCase {
    ...
    public void testReadFirstChild() throws Exception {
        Log.info("RMSStoreTest: testReadFirstChild");
        int childrenNumber = 10;
        int[] childIDs = new int[10];
        ...
        RmsRecordItem m = store.readFirstChild(FOLDER_PATH);

        assertTrue(m.getRecordId() == childIDs[9]);
        Log.info("RMSStoreTest: testReadFirstChild successful");
    }

    public void testReadNextChild() throws Exception {
        Log.info("RMSStoreTest: testReadNextChild");
        int childrenNumber = 10;
        int[] childIDs = new int[10];
        ...
        readChildrenIDs[0] = String.valueOf(store.readFirstChild(FOLDER_PATH).getRecordId());
        for (int i = 1; i < childrenNumber; i++) {
            readChildrenIDs[i] = String.valueOf(store.readNextChild(FOLDER_PATH).getRecordId());
        }
        
        assertTrue(isChildrenArrayMatching(childIDs, readChildrenIDs));

        Log.info("RMSStoreTest: testReadNextChild successful");
    }

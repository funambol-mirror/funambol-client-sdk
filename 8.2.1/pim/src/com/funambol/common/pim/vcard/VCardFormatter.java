...
public class VCardFormatter {
    ...
    throws PIMException
    {
        ...
        if (allFields || numValues > 0) {
            ...
            firstName   = pimUtils.escape(firstName);
            lastName    = pimUtils.escape(lastName);
            salutation  = pimUtils.escape(salutation);
            ...
            if (value != null) {
                ...
                switch (type) {
                    case Contact.ATTR_HOME:
                        home = pimUtils.escape(value);
                        break;
                    case Contact.ATTR_WORK:
                        work = pimUtils.escape(value);
                        break;
                    case Contact.ATTR_PAGER:
                        pager = pimUtils.escape(value);
                        break;
                    case Contact.ATTR_FAX:
                        fax = pimUtils.escape(value);
                        break;
                    case Contact.ATTR_MOBILE:
                        mobile = pimUtils.escape(value);
                        break;
                    case Contact.ATTR_OTHER:
                        other = pimUtils.escape(value);
                        break;
                    default:
    ...
    protected void formatAddress(String address[], String tag, OutputStream os) throws PIMException {
        ...
        String extendedAddress = pimUtils.escape(address[Contact.ADDR_EXTRA]);
        if (extendedAddress == null){
            extendedAddress = "";
        }

        String street = pimUtils.escape(address[Contact.ADDR_STREET]);
        if (street == null) {
            street = "";
        }

        String locality = pimUtils.escape(address[Contact.ADDR_LOCALITY]);
        if (locality == null) {
            locality = "";
        }

        String region = pimUtils.escape(address[Contact.ADDR_REGION]);
        if (region == null) {
            region = "";
        }

        String postalcode = pimUtils.escape(address[Contact.ADDR_POSTALCODE]);
        if (postalcode == null) {
            postalcode = "";
        }

        String country = pimUtils.escape(address[Contact.ADDR_COUNTRY]);
        if (country == null) {
            country = "";
        }
                                    ...
                                    boolean allFields) throws PIMException
    {
        ...
        String v = pimUtils.escape(value.toString());
        if (v == null) {
            v = "";
        }

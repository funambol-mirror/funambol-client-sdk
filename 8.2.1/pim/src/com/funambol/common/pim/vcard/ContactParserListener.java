...
public class ContactParserListener implements VCardSyntaxParserListener
{
    ...
    private PimUtils   pimUtils  = new PimUtils(defaultCharset);
    ...
    public void end() {
        ...
        Log.debug("This contact has: " + contact.countValues(Contact.EMAIL) + " emails");

        if (emailDefined) {
            PIMList list = contact.getPIMList();
            if (list.isSupportedField(Contact.EMAIL)) {
                for(int i=0;i<emails.length;++i) {
                    Log.debug("Adding email address: " + emails[i]);
    ...
    public void setFName(String content, ParamList plist,
        ...
        Log.trace("Setting Formatted Name");
        PIMList list = contact.getPIMList();
    ...
    public void setRevision(String content, ParamList plist,
        ...
        PIMList list = contact.getPIMList();
        if (list.isSupportedField(Contact.REVISION))
        {
            try {
                ...
                Log.error("Unrecognizable date format in revision field:" +content + "," + e.toString());
    ...
    public void setNickname(String content, ParamList plist,
        ...
        PIMList list = contact.getPIMList();
        if (list.isSupportedField(Contact.NICKNAME) &&
    ...
    public void setOrganization(String content, ParamList plist,
        ...
        Log.trace("Setting Organization");
    ...
    public void setBirthday(String content, ParamList plist,
        ...
        if (list.isSupportedField(Contact.BIRTHDAY))
        {
            try {
                ...
                Log.error("Unrecognizable date format in birthday field:" +content + "," + e.toString());
                throw new ParseException("Unrecognizable date format");
            }
        }
    }
    ...
    {
        PIMList list = contact.getPIMList();
        if (list.isSupportedField(Contact.UID))
        {
            if (content != null && content.length() > 0) {
    ...
    public void setName(String content, ParamList plist,
                        Token group) throws ParseException
    {
        Log.trace("Setting Name to " + content);

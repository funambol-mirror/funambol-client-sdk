...
public class VCardFormatter {
    ...
    private static final String TEL_HOME_TAG   = "TEL;VOICE;HOME";
    private static final String TEL_WORK_TAG   = "TEL;VOICE;WORK";
    private static final String TEL_PAGER_TAG  = "TEL;PAGER";
    private static final String FAX_WORK_TAG   = "TEL;FAX;WORK";
    private static final String TEL_MOBILE_TAG = "TEL;CELL";
    private static final String TEL_OTHER_TAG  = "TEL;VOICE";
    ...
    private static final String EMAIL_OTHER_TAG= "EMAIL;INTERNET";
    private static final String EMAIL_HOME_TAG = "EMAIL;INTERNET;HOME";
    private static final String EMAIL_WORK_TAG = "EMAIL;INTERNET;WORK";
    ...
    protected void formatFields(Contact contact, OutputStream os, boolean allFields) throws PIMException {
        ...
        if (list.isSupportedField(Contact.ORG)) {
        ...
        if (list.isSupportedField(Contact.PHOTO)) {
    ...
    throws PIMException
    {
        ...
        if (allFields || photo.length() > 0) {
            ...
            for(int i=0;i<photo.length();i+=64) {
                field.append(" ");
                if (i + 64 < photo.length()) {
                    field.append(photo.substring(i, i + 64));
                } else {
                    field.append(photo.substring(i));
                }
                field.append("\n");
            }

            println(os, field.toString());
        }
    }
    ...
    protected void formatAddress(String address[], String tag, OutputStream os) throws PIMException {
        ...
        String locality = escape(address[Contact.ADDR_LOCALITY]);
        if (locality == null) {
            locality = "";
        }

        String region = escape(address[Contact.ADDR_REGION]);
        if (region == null) {
            region = "";
        }

        String postalcode = escape(address[Contact.ADDR_POSTALCODE]);
        if (postalcode == null) {
            postalcode = "";
        }

        String country = escape(address[Contact.ADDR_COUNTRY]);
        if (country == null) {
            country = "";
        }
    ...
    private void println(OutputStream os, String msg) throws PIMException {
        // We need to escape the msg. The XML escaping is done by the
        // SyncManager, but we need to escape for VCard
        try {
            os.write(msg.getBytes());
            os.write("\n".getBytes());
        } catch (IOException ioe) {
            PIMException pe = new PIMException(ioe.toString());
            throw pe;
        }
    }
    ...
    protected String escape(String msg) {
        ...
        try {
            ...
            Log.error("[VCardFormatter] Cannot convert string" + e.toString());
        }
        return res.toString();
    }
}

...
public class CTPService extends Thread {
    ...
    public void run() {
                ...
                if (state == CONNECTED) {
                    ...
                    if ((authStatus == ST_UNAUTHORIZED) ||
                        ...
                        if (!heartbeatGenerator.isRunning()) {
                            ...
                            Log.debug("[CTPService.run] heartbeat generator is" +
        ...
        Log.debug("[CTPService.run]  Finishing CTPService thread");
    }
    ...
    protected void connect(int retry) throws IOException {
        ...
        try {
            ...
            sc = (SocketConnection) ConnectionManager.getInstance().open(uri);
    ...
    private CTPMessage createAuthMessage() {
        ...
        Log.debug("[CTPService.createAuthMessage] nonce is " + new String(Base64.encode(nonce)) + " ---- " + byteArrayToString(nonce));
        ...
        Log.debug("[CTPService.createAuthMessage] credentials " + new String(Base64.encode(credentials)) + " ---- " + byteArrayToString(credentials));
    ...
    protected void sendMessage(CTPMessage message) throws IOException {
        ...
        Log.debug("[CTPService.sendMessage]Sending message " + byteArrayToString(msgBytes));
        if (os == null) {
    ...
    protected CTPMessage receiveMessage() throws IOException {
        ...
        if (bytesRead != 2) {
            ...
            if (bytesRead != size) {
                ...
                Log.debug("[CTPService.receiveMessage]Received message " + byteArrayToString(rawMessage));
                return message;
            }
        }
    }

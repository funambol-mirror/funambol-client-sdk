...
char* WinTransportAgent::sendMessage(const char* msg, const unsigned int contentLength)
{
    ...
    for (numretries=0; numretries < MAX_RETRIES; numretries++) {
        ...
        //
        fireTransportEvent(realContentLength, SEND_DATA_BEGIN);
        ...
        fireTransportEvent(realContentLength, SEND_DATA_END);
    ...
    fireTransportEvent(responseLength, RECEIVE_DATA_BEGIN);
    ...
    fireTransportEvent(responseLength, RECEIVE_DATA_END);

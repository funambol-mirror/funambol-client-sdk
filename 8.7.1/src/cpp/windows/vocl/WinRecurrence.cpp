    ...
    switch(recType) {
        ...
        case winRecursDaily: {
            if(interval > 0)
                ...
                if (days) {
                    wstring d(days);
                    trim(d);
                    wsprintf(recurrence, TEXT("W1 %s #%ld"), d.c_str(), occurrences);
                    delete [] days;
                }
            }
            break;
        ...
        case winRecursWeekly: {
            if (dayofweek > 0) {
                ...
                if (days) {
                    ...
                    trim(d);
                    if(noEnd)
                        wsprintf(recurrence, TEXT("W%ld %s #0"), interval, d.c_str());
                    else
                        wsprintf(recurrence, TEXT("W%ld %s #%ld"), interval, d.c_str(), occurrences);
                    delete [] days;
                }
            }
            break;
        ...
        case winRecursMonthNth: {
            if(instance>0 && dayofweek>0) {
                days = daysOfWeekToString(dayofweek);
                wstring patternNoOccurrences = TEXT("MP%ld %ld+ %s #0");
                wstring patternOccurrences   = TEXT("MP%ld %ld+ %s #%ld");
                if (instance == 5) {
                    patternNoOccurrences = TEXT("MP%ld %ld- %s #0");
                    patternOccurrences   = TEXT("MP%ld %ld- %s #%ld");  
                    instance = 1;
                }
                ...
                if(days) {
                    ...
                    trim(d);
                    if(noEnd)
                        wsprintf(recurrence, patternNoOccurrences.c_str(), interval, instance, d.c_str());
                    else
                        wsprintf(recurrence, patternOccurrences.c_str(), interval, instance, d.c_str(), occurrences);
                    delete [] days;
                }
            }
            break;
        ...
        // YearNth = 6
        // The recurrence year nth are written as montlynth after 12 months. This
        // because there are issue in the outlook api to use the YM format
        case winRecursYearNth: {
            if(dayofweek>0 && instance>0) {
                ...
                if(days) {
                    ...
                    trim(d);
                    if(noEnd)
                        wsprintf(recurrence, patternNoOccurrences.c_str(), numberOfMonths, instance, d.c_str());
                        // wsprintf(recurrence, TEXT("YM1 %ld+ %s #0"), instance, days);
                    else
                        wsprintf(recurrence, patternOccurrences.c_str(), numberOfMonths, instance, d.c_str(), occurrences);
                        // wsprintf(recurrence, TEXT("YM1 %ld+ %s #%ld"), instance, days, occurrences);

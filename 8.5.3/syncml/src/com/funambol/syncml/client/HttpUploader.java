...
public class HttpUploader {
    ...
    private static final String LUID_PARAM = "LUID";

    private static final String AUTH_HEADER = "Authorization";
    private static final String AUTH_BASIC = "Basic";
    private static final String DEVID_HEADER = "deviceId";
     ...
     * its content is made available via the given stream.
     *
     ...
     * value is given, a default application/* type is used)
     */
    public int upload(String key, InputStream fileStream, int size, String contentType) throws HttpUploaderException {
        ...
        try {
            conn = connectionManager.openHttpConnection(url.toString());
            ...
            conn.setRequestProperty(DEVID_HEADER, devConf.devID);
        ...
        int status      = SyncMLStatus.GENERIC_ERROR;
    ...
    protected String createUploadUrl(String sessionId, String key) {
        ...
        // Append the LUID
        url.append("?").append(LUID_PARAM).append("=").append(key);

        // If we have a jsessionid we can specify it in the URL to speed up the
        // server work
        if (sessionId != null) {
            url.append(";").append(JSESSIONID_PARAM).append("=").append(sessionId);
        }
        return url.toString();
    }

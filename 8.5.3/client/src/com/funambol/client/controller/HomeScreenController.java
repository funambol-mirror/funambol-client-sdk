...
public class HomeScreenController extends SynchronizationController {
    ...
    public void attachToRunningSync(AppSyncSource appSource) {
        Log.trace(TAG_LOG, "Attaching to running sync for " + appSource.getName());
        // We need to lock the screen
        Vector sources = new Vector();
        sources.addElement(appSource);
        lockHomeScreen(sources);
        // Update the sync all button
        changeSyncLabelsOnSync(appSource);
        attachToSource(appSource);
    ...
    public void refreshMenuSelected() {
        ...
        /*
        if (!controller.getDialogController().promptNext(
                localization.getLanguage("dialog_refresh_warn"))) {
            return;
        }

        int direction = controller.getDialogController().showRefreshDirectionDialog();
        if (direction > 0) {
            int mask = controller.getDialogController().showRefreshTypeDialog(direction);
            if (mask > 0) {
                controller.showScreen(homeScreen, Controller.HOME_SCREEN_ID);
                refresh(mask, direction);
            }
        }*/
    ...
    private void computeVisibleItems() {
        ...
        int realSize = computeNumberOfVisibleSources();
        if (realSize == 0) {
    ...
    private boolean isVisible(AppSyncSource appSource) {
        boolean visible = (appSource != null && appSource.isWorking() && appSource.getConfig().getActive());

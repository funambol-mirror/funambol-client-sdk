...
public class SynchronizationController implements ConnectionListener, SyncEngineListener {
    ...
    SynchronizationController(Controller controller, Screen screen) {
        ...
        this.syncScheduler = new SyncScheduler(engine);
    ...
    public synchronized void refreshSources(Vector syncSources, int direction) {
        ...
        forceSynchronization(syncType, syncSources, true, direction, 0, false);
    }
                                                     ...
                                                     int delay, boolean fromOutside)
    {
        ...
        // if the syncType is scheduled, we shall skip all the sources which
        // have a device full error
        if (SCHEDULED.equals(syncType)) {
            ...
            for(int i=0;i<syncSources.size();++i) {
                ...
                // in this session, then we include the source, othterwise we
                // skip it
                if (appSource.getConfig().getLastSyncStatus() != SyncListener.DEVICE_FULL_ERROR ||
        ...
        if (screen == null && sourcesWithQuestion.size() > 0) {
            ...
            if (sourcesWithQuestion.size() == 0) {
                ...
                continueSynchronizationAfterDialogCheck(syncType, filteredSources, 
    ...
    public synchronized void continueSynchronizationAfterDialogCheck(String syncType,
    ...
    public void noCredentials() {
        controller.getDialogController().showMessage(screen,
                localization.getLanguage("message_login_required"));

    }

    public void noSources() {
        controller.getDialogController().showMessage(screen,
                localization.getLanguage("message_nothing_to_sync"));

    }

    public void noConnection() {
        controller.getDialogController().showMessage(screen,
                localization.getLanguage("message_radio_off"));

    }

    public void noSignal() {
        controller.getDialogController().showMessage(screen,
                localization.getLanguage("message_no_signal"));
    ...
    public void endSync(Vector sources, boolean hadErrors) {
        ...
        if (hadErrors && showTCPAlert) {
            ...
            controller.getDialogController().showMessage(screen,
                    localization.getLanguage("message_enter_TCP_settings"));
        }
        ...
        if (deviceFullSources != null) {
            ...
            for(int i=0;i<deviceFullSources.size();++i) {
                ...
                Log.error(TAG_LOG, "Manual sync: server full for source " + appSource.getName());
    ...
    public void serverOperationFailed() {
        ...
        controller.getDialogController().showMessage(screen,
                localization.getLanguage("message_not_send_to_server"));
    }

...
int iPhoneEvent::parse(const StringBuffer& dataString) {
	...
	if (startDateValue.length() > 0 && endDateValue.length() > 0) {
		...
		if (element = getVObjectPropertyValue(vo, "X-FUNAMBOL-ALLDAY")) {
            ...
            //LOG.debug("isAllDay = %s", isAllDay? "true":"false");
		}      
		...
		if (isAllDay) {			
            ...
            NSComparisonResult res = [sdate compare:edate];
    ...
    //NSString* tmp = [sdate description];
    //LOG.debug("start date = %s", [tmp UTF8String]);
    //tmp = [edate description];
    //LOG.debug("end date = %s", [tmp UTF8String]);
	...
	if(element = getVObjectPropertyValue(vo, "PRIORITY")) {         // anyway it's not supported by iphone
	...
	if (element = getVObjectPropertyValue(vo, "CLASS")) {          // anyway it's not supported by iphone
	...
	if(element = getVObjectPropertyValue(vo, "X-MICROSOFT-CDO-BUSYSTATUS")) {         // anyway it's not supported by iphone
...
StringBuffer iPhoneEvent::toString() {
    ...
    if (getReminder()) {		
        ...
        LOG.debug("alarm1 time = %s", alarmDate.c_str());
    ...
    //
	// BusyStatus, Importance, Sensitivity: 

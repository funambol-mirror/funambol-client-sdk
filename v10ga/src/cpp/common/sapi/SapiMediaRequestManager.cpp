...
ESapiMediaRequestStatus SapiMediaRequestManager::login(const char* device_id, time_t* serverTime)
{
    int status = 0;
    ...
    if ((status = jsonSapiMediaObjectParser->parseLogin(sapiLoginResponse, sessionID, serverTime, &parserStatus)) == false) {

...
public class SyncManager {
    ...
    public synchronized void sync(SyncSource src, int syncMode,
        ...
        try {
            ...
            getSyncListenerFromSource(src).dataReceived(transportAgent.getResponseDate(),
                    response.length());
            ...
            getSyncListenerFromSource(src).endConnecting(alertCode);
    ...
    private void appendStatusTags(StringBuffer out) {
        ...
        for (int idx = 0; idx < l; idx++) {
            ...
            if (SyncML.TAG_ALERT.equals(status.getCmd())) {
                out.append(formatter.formatAlertStatus(status, source.getNextAnchor()));

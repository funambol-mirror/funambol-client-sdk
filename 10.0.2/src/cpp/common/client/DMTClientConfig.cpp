...
bool DMTClientConfig::readRootConfig() {
    ...
    ManagementNode* rootNode = dmt->readManagementNode(rootContext);
    if (!rootNode) {
        return false;
    }
...
bool DMTClientConfig::readDeviceConfig(ManagementNode& n, bool server) {
    ...
    //
    if (server) {
        sprintf(nodeName, "%s%s", syncMLContext, CONTEXT_DATASTORES);
        node = dmt->readManagementNode(nodeName);
        if (node) {
...
bool DMTClientConfig::readSourceConfig(int i,
    ...
    while (kvp.null() == false) {
        ...
        if (kvp.getKey() != PROPERTY_SOURCE_NAME && kvp.getKey() != PROPERTY_SOURCE_URI && 
            ...
            kvp.getKey() != PROPERTY_SOURCE_ENABLED && kvp.getKey() != PROPERTY_SOURCE_ENCRYPTION &&
...
// SERVER CONFIG ONLY!
bool DMTClientConfig::readDataStoresConfig(ManagementNode& dataStoresNode) {
    ...
    for (int i=0; i<numDataStores; i++) {
        ...
        ManagementNode* node = dmt->readManagementNode(fullName.c_str(), dataStoreNames[i]);
        if (node) {
...
// SERVER CONFIG ONLY!
DataStore* DMTClientConfig::readDataStoreConfig(ManagementNode& dataStoreNode) {
...
// SERVER CONFIG ONLY!
void DMTClientConfig::saveDataStoresConfig(ManagementNode& dataStoresNode) {
    ...
    for (int i=0; i < dataStores->size(); i++) {
        ...
        if (dataStore) {
            ...
            ManagementNode* node = dmt->readManagementNode(fullName.c_str(), sourceRef.c_str());
            if (node) {
...
// SERVER CONFIG ONLY!
void DMTClientConfig::saveDataStoreConfig(ManagementNode& dataStoreNode, DataStore* dataStore) {
...
bool DMTClientConfig::readSapiConfig(ManagementNode& n) {
    ...
    // read all properties
    sprintf(nodeName, "%s%s", syncMLContext, CONTEXT_SAPI);
    node = dmt->readManagementNode(nodeName);
    if (node) {
        if (!readSapiConfig(n, *node)) {
            ret = false;
        }
        delete node;
        node = NULL;
...
bool DMTClientConfig::readSapiConfig(ManagementNode& /* syncMLNode */, ManagementNode& sapiNode) {

    // All properties are in the extra-map
    StringMap* stringMap = sapiNode.readAllProperties();
    KeyValuePair kvp = stringMap->front(); 
    
    while (kvp.null() == false) {
        if (kvp.getKey() != PROPERTY_DUMMY_KEY) {
            sapiConfig.setProperty(kvp.getKey().c_str(), kvp.getValue().c_str());
        }
        kvp = stringMap->next();
    }
    delete stringMap;
...
void DMTClientConfig::saveSapiConfig(ManagementNode& n) {
    ...
    node = dmt->readManagementNode(nodeName);
    if (node) {
        saveSapiConfig(n, *node);
        delete node;
        node = NULL;
    }
}

void DMTClientConfig::saveSapiConfig(ManagementNode& /* syncMLNode */, ManagementNode& sapiNode) {
    
    //LOG.debug("entering %s", __FUNCTION__);

    // All properties are in the extra-map
    StringMap& stringMap = const_cast<StringMap &>(sapiConfig.getExtraProps());
    KeyValuePair kvp = stringMap.front(); 

    while (kvp.null() == false) {
        if (kvp.getKey() != PROPERTY_DUMMY_KEY) {
            sapiNode.setPropertyValue(kvp.getKey().c_str(), kvp.getValue().c_str());  
        }
        kvp = stringMap.next();
    }
}

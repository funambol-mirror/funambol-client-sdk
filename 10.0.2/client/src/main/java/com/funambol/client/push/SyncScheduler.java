...
public class SyncScheduler {
    ...
    private void checkQueue(SchedulerRequest schedulReq){
        ...
        if (Log.isLoggable(Log.TRACE)) {
            Log.trace(TAG_LOG, "requests in queue: " + queueSize);
        }
                    ...
                    if(!requestQueue.isEmpty()){
                        ...
                        for (int pos = 0; pos<queueSize; pos++) {
                            ...
                            if (srCombined.getExpirationTime() < expTime) {
                                if (Log.isLoggable(Log.TRACE)) {
                                    Log.trace(TAG_LOG, "Putting combined request in "+pos+"position");
                                }
            ...
            if (!requestCombined) {
                ...
                for (int i=0; i< queueSize; i++) {
                    ...
                    if (schedulReq.getExpirationTime() < expTime) {
                        if (Log.isLoggable(Log.TRACE)) {
                            Log.trace(TAG_LOG, "putting new request in "+i+" position");
                        }
        ...
        if(requestQueue.size() > 0) {
            for (int i=0; i< requestQueue.size(); i++) {
                ...
                if (merged != null) {
                    ...
                    if (Log.isLoggable(Log.TRACE)) {
                        Log.trace(TAG_LOG, "was scheduled at: " + new Date(or.getExpirationTime()));
                    }
    ...
    private void setTimer(long startTime){
        ...
        rq = new RequestTimer();
        if (Log.isLoggable(Log.DEBUG)) {
            Log.debug(TAG_LOG, "timer set at: "+new Date(startTime));
        }
        try{
            scheduleTimer.schedule(rq, new Date(startTime));
        }catch(Exception ex){
            Log.error(TAG_LOG, "ex scheduling timer: ", ex);
        }
    }
    ...
    private void callListener(SyncRequest syncRequest){
        ...
        if(!requestQueue.isEmpty()){
            ...
            if (Log.isLoggable(Log.DEBUG)) {
                Log.debug(TAG_LOG, "starting sync... reschedule timer for next request at: " + 
                                   new Date(nextTimer));
            }
            setTimer(nextTimer);
        }
    ...
    private class SchedulerRequest{
        ...
        SchedulerRequest combine(SchedulerRequest queuedRequest) {
            ...
            if (merged != null) {
                if (Log.isLoggable(Log.TRACE)) {
                    Log.trace(TAG_LOG, "Requests can be combined and scheduled at: "

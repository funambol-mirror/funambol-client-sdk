...
public class SyncManager {
    ...
    throws SyncException {
        ...
        try {
            ...
            listener.startSyncing(alertCode);
    ...
    /* 
     * This method used only for 3GSM demo.
     * Set the maxmessagesize for incoming emails with attachments
                ...
                if( statusOf.equals(status.getCmd()) ) {
                    switch (status.getStatus()) {
                        ...
                        case SyncMLStatus.NOT_FOUND:                    // 404
                            Log.error(this,"Source URI not found on server: "
                                    + source.getSourceUri());
                            throw new SyncException(
                    ...
                    || cmd.equals(SyncML.TAG_SYNC)) {
                        if(status.getStatus() == 506){
                            String msg = "Server responded "
                                    +status.getStatus()
                                    +" to command "
                                    +cmd + " [" + status.getStatusDataMessage()+ "]"; //XXX
                            Log.error(msg);
    ...
    throws SyncException {
        ...
        try {
            ...
            Log.error("[processModification] Invalid Sync command: "
                    +e.toString());
            e.printStackTrace();
            throw new SyncException(
                    SyncException.SERVER_ERROR,
        ...
        if(!locuri.equals(source.getName())) {
            Log.error("[processModification] Invalid uri: '" + locuri
